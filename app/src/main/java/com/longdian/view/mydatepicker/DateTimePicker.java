package com.longdian.view.mydatepicker;
...
import com.longdian.view.mydatepicker.util.DateUtils;
import com.longdian.view.mydatepicker.util.LogUtils;
import com.longdian.view.mydatepicker.widget.WheelView;
...
public class DateTimePicker extends WheelPicker {
    ...
    @IntDef(value = {NONE, YEAR_MONTH_DAY, YEAR, YEAR_MONTH, MONTH_DAY})
    ...
    public void setDateRangeStart(int startYearOrMonth, int startMonthOrDay) {
        ...
        if (dateMode == YEAR_MONTH || dateMode == YEAR) {
    ...
    public String getSelectedYear() {
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH || dateMode == YEAR) {
    ...
    protected View makeCenterView() {
        // 如果未设置默认项，则需要在此初始化数据
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH || dateMode == YEAR) && years.size() == 0) {
...
//                    selectedMonthIndex = 0;//重置月份索引
        ...
        if (dateMode != NONE) {
            ...
            if (!TextUtils.isEmpty(monthLabel) && dateMode != YEAR) {
                TextView labelView = createLabelView();
                labelView.setTextSize(textSize);
