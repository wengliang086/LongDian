...
public class HeatAnalysisFragment1 extends Fragment {
    ...
    private List<List<String>> createData(List<Map<String, String>> list) {
        ...
        datas.add(Arrays.asList("换热站名称", "实际面积(m²)", "日期", "供热量", "运行单耗(T/m²)", "统计时间(小时)", "开始时间", "结束时间"));
        for (int i = 0; i < list.size(); i++) {
            Map<String, String> s = list.get(i);
            List<String> row = Arrays.asList(s.get("station_name"), s.get("real_area"), s.get("dt"), s.get("qqi_total"), s.get("yxdhstr"), s.get("hours"), s.get("bDateTime"), s.get("eDateTime"));
            datas.add(row);
        }
        return datas;
    }
